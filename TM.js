...
#chat-input-textbox::-webkit-input-placeholder,
...
${qss.filter(qs => ![`.hide-when-print.sticky`, `#elements-in-action-buttons`].includes(qs)).map(qs => `.fullscreen-hide ${qs}`).join(',\n')} {
  ...
  const setPluginsState = async (enable = true) => {
    const desiredMaybePluginNames = ['Memory', 'Personal Finance', 'Javascript Interpreter', 'Search Google'];
    ...
    pluginItems.filter(e => desiredMaybePluginNames.some(desiredPluginName => e.innerText.toLowerCase().includes(desiredPluginName.toLowerCase()))).forEach(e => {
